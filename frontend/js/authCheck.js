...
const isProtectedRoute = path.endsWith('dashboard.html') || path.endsWith('profile.html') || path.endsWith('stocks.html');

if (!localStorage.getItem('token') && isProtectedRoute) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        overlay.innerHTML = `
            <div class="card panel reveal" style="pointer-events: auto; user-select: auto; width: 380px; padding: 2.5rem; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1);">
                ...
                <form id="overlayLoginForm" autocomplete="off" style="display: flex; flex-direction: column; gap: 1.25rem;">
                    <div>
                        <label style="font-weight: 600; margin-bottom: 0.5rem; display: block;">Email</label>
                        <input type="email" name="email" required autocomplete="off" style="width: 100%; padding: 0.75rem; border-radius: 6px; border: 1px solid rgba(255,255,255,0.2); background: rgba(255,255,255,0.05); color: #fff; font-family: inherit;" />
                    </div>
                    <div>
                        <label style="font-weight: 600; margin-bottom: 0.5rem; display: block;">Password</label>
                        <input type="password" name="password" required autocomplete="new-password" style="width: 100%; padding: 0.75rem; border-radius: 6px; border: 1px solid rgba(255,255,255,0.2); background: rgba(255,255,255,0.05); color: #fff; font-family: inherit;" />
                    </div>
                ...
                <a href="http://localhost:3000/api/auth/google" class="btn btn-outline" style="width: 100%; display: block; text-align: center; text-decoration: none; box-sizing: border-box;">Sign in with Google</a>
        ...
        document.getElementById('overlayLoginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Reload the page to actually fetch the protected data (profile/dashboard)
