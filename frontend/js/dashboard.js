...
searchDateBtn.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/stocks/search?date=${encodeURIComponent(date)}`);
    const data = await response.json();
...
runAnalysisBtn.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/stocks/analysis', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
    const data = await response.json();
...
loadStocksBtn.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/stocks', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
    const data = await response.json();
